...
- **Q6**: ~~Tighten the CSP — drop `style-src 'unsafe-inline'`.~~ **Done 2026-05-02.** Audited the production bundle and our source: zero `<style>` tags in `dist/`, zero `style={{}}` props in our code. The only inline-style consumer is `react-router-dom`'s default error boundary (`<h3 style={{fontStyle:"italic"}}>`). Split the directive into modern halves: `style-src-elem 'self'` blocks injected `<style>` elements (defence in depth against the findings sanitiser), `style-src-attr 'unsafe-inline'` keeps the router's failure path renderable. Added `script-src 'self'`, `object-src 'none'`, `base-uri 'self'`, `frame-ancestors 'none'` while we were in there. Regression test `csp_does_not_relax_style_src_with_unsafe_inline` in `tests/readonly.rs` fails the build if anyone reverts to the broad form.
